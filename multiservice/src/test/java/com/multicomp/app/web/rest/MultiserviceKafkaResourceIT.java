...
import com.multicomp.app.config.EmbeddedKafka;
import com.multicomp.app.config.KafkaSseConsumer;
import com.multicomp.app.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class MultiserviceKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/multiservice-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/multiservice-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/multiservice-kafka/unregister"));
                return;
            }
        }
